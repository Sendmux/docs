...
Mailbox credentials are scoped to one mailbox. Root API keys are rejected on Mailbox API endpoints. The same credential is also accepted as the mailbox password for SMTP submission, IMAP retrieval, and POP3 retrieval.
...
```json
{
  ...
  "data": {
    ...
    "limits": {
      ...
      "attachment_upload_bytes_max": 50000000
    },
    "gated": {
...
Use identities to discover which sender addresses and default signatures are
available to the authenticated mailbox.
...
<Note>
  The Mailbox API exposes identities as read-only. It does not create, update,
  or delete sender identities.
</Note>
...
| `undo_status`    | Match `pending`, `final`, or `canceled`.       |
...
Use batch update to set allowed flags on up to 100 messages in one state-safe
request. Only `seen` and `flagged` are mutable.
...
Use `POST /mailbox/attachments:upload` for larger files. Send the raw bytes in
the request body and pass the returned `blob_id` to `POST /mailbox/messages/send`.
