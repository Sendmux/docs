...
Authenticate with a mailbox credential, [REST OAuth access token](/developer-tools/oauth), or agent access token from Sendmux. Pass it as a Bearer token in the `Authorization` header.
...
Use `GET /mailbox/connection` to [test an integration connection](/developer-tools/integration-connections) without checking storage or selecting a mailbox. `GET /mailbox/me` keeps its existing mailbox details and storage-usage behaviour.

| Endpoint                                                         | Purpose                                                           |
| ---------------------------------------------------------------- | ----------------------------------------------------------------- |
| `GET /mailbox/connection`                                       | Test authentication without selecting or reading a mailbox.       |
