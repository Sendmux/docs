...
<Note>
  New Free teams include **1 webhook**. Your current limit appears on the
  **Webhooks** page. Free limits are fixed. Delete an unused webhook or upgrade
  to Pro when you need more capacity. See [Team limits](/account/team-limits).
</Note>

## Webhooks or realtime events

Use webhooks when your backend should receive signed event POSTs, even while your app is offline. Use the [Mailbox API event stream](/mailbox-api/introduction#event-stream) when a mailbox client can keep a live Server-Sent Events connection open and needs immediate received-message updates.
...
Verify the signature before acting on an event. See [Verify webhook signatures](/webhooks/verify-signatures).
...
<Columns cols={2}>
  ...
  <Card title="Webhook replay" icon="rotate-ccw" href="/webhooks/replay">
