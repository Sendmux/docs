...
<Note>
  ...
  message and records it in [Logs](/monitoring/dashboard-logs#delivery-logs)
  rather than returning a bounce.
</Note>
...
<Steps>
  <Step title="Add your cold email accounts">
    ...
    **Gmail**, or **Outlook / Microsoft 365**. Run **Test Connection** on SMTP
    ...
    [Import accounts from a file](/sending/accounts#import-accounts-from-a-file).
    ...
    Full reference:
    [Add your own account](/sending/accounts#add-your-own-account) and
    [Advanced settings](/sending/accounts#advanced-settings).
  ...
  <Step title="Create a sending key, for SMTP and HTTP">
    ...
    once. See [Create a key](/account/api-keys#create-a-key) and
    [Key types](/account/api-keys#key-types).

    Then send:

    - **SMTP.** Host `smtp.sendmux.ai`, port `587` with STARTTLS, password is
      your sending key. See [Sending by SMTP](/sending/smtp).
    - **HTTP.** `POST` to the Sending API. See
      [Sending by HTTP](/sending/http).
  ...
  <Step title="Or connect an AI client over MCP">
    ...
    secrets. See [Connected apps](/account/api-keys#connected-apps) and the next
    section.
  </Step>
</Steps>
...
<Steps>
  <Step title="Add the server">
    ...
    in [Hosted OAuth
    clients](/ai-integrations/mcp/clients#hosted-oauth-clients).
  </Step>
  ...
  <Step title="Check the tools appear">
    Confirm the sending tools are listed. See [Verify the
    connection](/ai-integrations/mcp/clients#verify-the-connection).
  </Step>
</Steps>
...
For what each surface covers and how long tokens last, see [MCP](/ai-integrations/mcp).
...
Distribution percentage weights traffic between accounts that are eligible for the same send. Delivery groups go further and keep a stream of traffic on a chosen subset of accounts. The key you authenticate with decides which applies. See [How routing works](/sending/accounts/delivery-groups#how-routing-works).
...
You only need a Sendmux domain and mailbox when you want replies inside Sendmux, so an agent can read and act on them. That is also when the **Mailbox API** grant becomes worth adding to your MCP connection. See [Domain management](/domains) and [Mailboxes](/mailboxes).
...
<Accordion title="Do I need a mailbox to use MCP for sending?">
  ...
  need to create one. Add the Mailbox API product line only when you want an
  agent to read replies.
</Accordion>

<Accordion title="How many sending accounts can I add?">
  Free includes one account you add or connect yourself. Pro has no plan-level
  sending-account count limit. The managed account does not count towards that.
  See [Account limits](/sending/accounts#account-limits) and [Team
  limits](/account/team-limits#resource-limits).
</Accordion>
...
<Accordion title="Can I use the free managed account for cold outreach?">
  ...
  <Accordion title="How do I see what was delivered?">
    ...
    the result. See [Filter logs](/monitoring/dashboard-logs#filter-logs).
  </Accordion>
</AccordionGroup>
...
<Columns cols={2}>
  ...
  <Card title="MCP clients" icon="plug" href="/ai-integrations/mcp/clients">
