---
title: "Run cold email outreach and GTM from your own sending accounts"
...
"og:title": "Run cold email outreach and GTM from your own sending accounts"
...
| Send through the managed account | Yes |
...
<Steps>
  <Step title="Add your cold email accounts">
    ...
    **Gmail**, or **Outlook / Microsoft 365**. Run **Test Connection** on SMTP
    ...
    <Frame caption="Per-account quotas accept a fixed cap or a range.">
      <img
        src="/images/integrations/email-marketing/sendmux-account-quotas.png"
        alt="Sendmux per-account quota fields for per-second, per-minute, per-hour and per-day limits"
      />
    </Frame>
  ...
  <Step title="Send from your tool or your agent">
    ...
    - **An AI client over MCP.** See the next section.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Authorise the connection">
    Sendmux asks you to pick a team and the product surfaces the client may use.
  </Step>
...
<Warning>
  **Grant Sending only.** Every product line is preselected on the authorisation
  screen. Clear **Management** and **Mailbox** so **Sending** is the only one
  left. A Sending-only grant never asks you to choose mailboxes, and you do not
  need to create one. Add **Mailbox** later if you also want the agent to read
  replies.
</Warning>
...
You only need a Sendmux domain and mailbox when you want replies inside Sendmux, so an agent can read and act on them. That is also when the **Mailbox** grant becomes worth adding to your MCP connection. See [Domain management](/guides/domain-management) and [Mailboxes](/guides/mailboxes).
...
- Sending key stored in a secrets manager and scoped to the right providers.
- MCP grant limited to Sending, unless you deliberately want mailbox access.
...
<AccordionGroup>
  ...
  <Accordion title="Do I need a mailbox to use MCP for sending?">
    ...
    need to create one. Add the Mailbox surface only when you want an agent to
    read replies.
  </Accordion>
