...
The Sending API uses the same error envelope as the [Management API](/api-reference/errors): `code`, `message`, `param`, `doc_url`, `retryable`, accumulated `errors[]`, `X-Request-Id`, and `Retry-After` on `429` or `503`. This page documents only the codes the Sending API emits.
...
**HTTP 413**: The request body exceeds the maximum allowed size. `html_body` and `text_body` are capped at 10MB each. The full request body, including base64-encoded attachments, must be 25MB or less. Attachments are also bounded by the 10-attachment maximum.

### no_active_providers

**HTTP 503**: Your team has no active sending account. Every team starts with the managed Amazon SES account active, so this usually means it was deactivated and no other provider is active. Reactivate the managed account, or add and activate a provider under **Accounts**. See [Provider setup](/guides/sending-accounts). `retryable` is `true`, but retrying will keep failing until an account is active.
