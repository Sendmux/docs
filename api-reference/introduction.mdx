...
keywords: ["management API", "REST API", "authentication", "API keys", "errors", "snake_case"]
---
...
Every `GET` that returns a single resource emits a weak `ETag`. Send it back on the next request to avoid transferring unchanged data, and use `If-Match` on writes for optimistic concurrency control.
...
- **`If-Match`** on a `PATCH` or `PUT` — if the resource changed since you fetched it (a stale `ETag`), the API responds `409 conflict` so you don't overwrite a concurrent update. Re-fetch, reapply your change, and retry.
