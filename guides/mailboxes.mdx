...
The Management API exposes the same rules for automation: `GET`/`PUT /mailboxes/{public_id}/filters` for one mailbox, and `GET`/`PUT /domains/{public_id}/filters` to apply a rule set across every mailbox on a domain. Each `PUT` replaces the whole rule set in a single atomic operation. See the [Management API reference](/api/introduction).
