...
<Steps>
  ...
  <Step title="Verify">
    Select **Verify** after the records have propagated. The domain is ready
    when all records show as ready.
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="SPF is failing">
    Keep only one SPF record on a hostname. Merge all `include:` values into the
    same TXT record.
  </Accordion>
