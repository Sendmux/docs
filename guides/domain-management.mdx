...
<Info>
  Incoming email is billed at the standard `$0.15 / 1,000 emails` rate. To
  control unwanted inbound volume, use mailbox or domain sender filters to allow
  trusted senders or deny noisy ones.
</Info>

<Note>
  Each team starts with **25 domains**. The Domains page shows current usage and
  displays **Request increase** when usage reaches 80% of your current limit.
  See [Team limits](/guides/team-limits).
</Note>

## Add and verify a domain

<Steps>
  <Step title="Call POST /domains">
    ...
    If your team has reached its domain limit, the API returns `409 limit_exceeded`.
  ...
  <Step title="Place every DNS record at your DNS provider">
    See [Required DNS records](#required-dns-records) below for what each record does. Place them exactly as returned — Sendmux's verification check looks for byte-exact values.
  </Step>
...
| TXT    | `mail.acme.com`                         | `v=spf1 include:amazonses.com ~all` | SPF. Authorises Amazon SES to send for the domain. If you use your own upstream provider as well, merge its `include:` mechanism into this same record. |
...
<AccordionGroup>
  <Accordion title="verification_txt: false">
  ...
  <Accordion title="spf: false">
    ...
    - If your apex already has `v=spf1 include:_spf.google.com ~all`, add the Amazon SES include to the same record: `v=spf1 include:amazonses.com include:_spf.google.com ~all`.
    - Check for typos — SPF is whitespace-sensitive.

  </Accordion>
  <Accordion title="dmarc: false">
  ...
  <Accordion title='ses_dkim_status: "failed" or "temporary_failure"'>
    ...
    - If `failed` persists for more than 72 hours with the records in place, contact support at [contact@sendmux.ai](mailto:contact@sendmux.ai).
...
<Note>
  Keys provisioned with `root:readonly` carry `domain.read` so monitoring
  integrations can list your domains without being able to modify them.
</Note>
