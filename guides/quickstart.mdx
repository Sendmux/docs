...
<Steps>
  ...
  <Step title="Create a mailbox">
    A mailbox is a single sending address bound to a verified domain (or to the shared `@myagent.mx` namespace). Each mailbox gets its own credentials and can have its own delivery rules.
    ...
    **In the dashboard**: open **Mailboxes** and click **Create mailbox**. Pick the verified domain from step 1, choose a local part (for example, `notifications`), and click **Create**.
    ...
    The dashboard shows two one-shot secrets that **cannot be retrieved later** — copy them now:
