...
Pre-claim agent tokens include `mailbox.read` and `email.receive`, not `email.send`. After the owner joins and approves sending, the agent can request a Sending-resource `smx_agent_` token through [agent access](/guides/agent-access).
