...
The authorisation-server metadata advertises the token and revocation endpoints at the top level. Its `agent_auth` block includes `identity_endpoint`, `x_sendmux_proof_of_work_challenge_endpoint`, `x_sendmux_identity_invite_endpoint`, and `identity_types_supported`. The proof-of-work field points to the registration challenge endpoint. The invite field is the Sendmux-specific owner invite extension.
