...
Sendmux cannot recover or reveal the raw `claim_token` later because the service stores only a hash. If the raw claim token is lost, rerun registration and invite with a fresh agent identity.
