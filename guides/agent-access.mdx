...
When registering a new agent identity, save the agent credential bundle in the safest secret store your environment provides before reporting success or losing context. Later MCP, CLI, SDK, mailbox, and sending work depends on this bundle.

Store `claim_token`, `registration_id`, `mailbox.email`, `claim_token_expires`, `token_endpoint`, `app_resource`, and `sending_resource`. Also keep `identity_assertion` or the pre-claim `smx_agent_` access token available until the owner invite returns `202`. If no secure store is available, ask the user where to store these credentials before continuing.
...
- Save the agent credential bundle in a secure store before reporting success.
...
Before exchanging the identity assertion, inviting the owner, or reporting that setup succeeded, save the agent credential bundle in a secure store controlled by the user or agent environment, such as 1Password, an OS keychain, or the agent platform's encrypted secret store. This bundle is required later for MCP, CLI, SDK, mailbox, and sending work.
