...
Agent access lets an AI agent start with one constrained `@myagent.mx` mailbox before a person joins the team. The agent can read and receive mail for that mailbox, ask Sendmux to invite the human owner, and use owner-approved sending after the owner joins and grants it.
...
<Steps>
  ...
  <Step title="Create an anonymous agent identity">
    Register once to get an identity assertion, a claim token, and a
    constrained agent mailbox.
  </Step>
...
If the owner has joined but not approved sending yet, the token endpoint returns `503 authorization_pending` with `Retry-After: 10`.
...
| `https://app.sendmux.ai/agent-auth/oauth2/token` | 503 | `authorization_pending` | Owner approval for sending is not complete. | Retry after the `Retry-After` header. |
