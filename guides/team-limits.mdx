...
| Sending accounts | 200 | [Provider setup](/guides/sending-accounts) |
...
When a resource reaches **80%** of its current limit, Sendmux shows **Request increase** near the usage text on that page. Submit the request once. The page then shows the pending request state.
...
Sending through the managed Amazon SES account still follows its daily sending limit. Your own providers may also enforce provider-side sending limits.
...
<Columns cols={2}>
  ...
  <Card title="Provider setup" icon="send" href="/guides/sending-accounts">
