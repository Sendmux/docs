...
<Info>
  ...
  MCP, or use agent access first, then let the skill choose the right
...
Use send-capable `smx_mbx_` keys for Sending work. Use `smx_mbx_` keys or scoped `smx_agent_` tokens for allowed Mailbox work. Use team-scoped `smx_root_` keys for Management work. Pre-claim `smx_agent_` tokens do not include `email.send`.
