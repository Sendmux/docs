...
<Info>
  ...
  for the task. If you use agent access, save the returned `claim_token` in a
  secure store such as 1Password, an OS keychain, or the agent platform's
  encrypted secret store before inviting the owner.
</Info>
...
When an agent has no Sendmux credential, start with [agent access](/guides/agent-access): request a registration challenge, solve it, register the mailbox at the `identity_endpoint` with `proof_of_work`, save the returned `claim_token` in a secure store such as 1Password, an OS keychain, or the agent platform's encrypted secret store, exchange the identity assertion, then invite the human owner. The claim token is required later for owner-approved sending and cannot be recovered if the raw value is lost. If any agent-auth endpoint returns `429`, use `Retry-After` or `retry_after` before retrying.
