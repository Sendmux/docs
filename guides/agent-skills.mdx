...
<Info>
  ...
  for the task. If you use agent access, save the returned `claim_token` for
  post-approval exchange, but keep the `identity_assertion` or pre-claim token
  available until the owner invite returns `202`.
</Info>
