...
The documentation MCP exposes one tool, `search_sendmux`.
...
Product MCP can expose any combination of three surfaces:

| Surface | Tools | Credential |
| --- | ---: | --- |
| Mailbox | 21 | Hosted grant with Mailbox access, or an `smx_mbx_` key locally |
| Management | 20 | Hosted grant with Management access, or an `smx_root_` key locally |
| Sending | 2 | Hosted grant with Sending access, or an `smx_mbx_` key locally |

Choose the hosted connection when your client supports remote MCP and OAuth. Choose local stdio when your client starts MCP servers on your machine. Choose self-hosted HTTP when you run `sendmux-mcp` behind your own private endpoint.

## Hosted connection

Hosted product MCP uses OAuth. It does not create manual `smx_root_` or `smx_mbx_` keys for the agent.
...
| Too many tools in one client | Connect only the surfaces you need, or disable unused tools in the client. Sendmux exposes 43 tools across all product surfaces. |
