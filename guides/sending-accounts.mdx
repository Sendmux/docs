...
Use CSV import to create or update many custom accounts at once.
...
<Steps>
  ...
  <Step title="Download the template">
    Select **Download Template**. Keep the header row unchanged and add one
    account per row.
  </Step>
  <Step title="Review the rows">
    Upload the CSV. Sendmux shows valid and invalid rows before it starts. Only
...
<Warning>
  CSV import can update a connected Gmail or Outlook account, but it cannot
  authorise a new one. Use **Add Account** to connect a new account that needs
  to send.
</Warning>
...
- Select **Accounts** > **Export Accounts** to download the custom accounts listed in **Accounts**. Passwords are always blank in the export.
