...
<Info>
  For API-key authentication, Sending clients accept a send-capable `smx_mbx_` key or owner-approved Sending-resource `smx_agent_` token. Mailbox clients accept
...
- Go 1.23 or newer.
- An API key or [REST OAuth grant](/developer-tools/oauth) for the surface you are calling.
...
```bash
go get sendmux.ai/go/v3@latest
```

## OAuth access tokens

Use `NewWithAccessToken` for a bare token or `NewWithTokenProvider` for a function that receives the request context.
...
func oauthClient() (*mailbox.Client, error) {
    return mailbox.NewWithTokenProvider(func(ctx context.Context) (string, error) {
        return os.Getenv("SENDMUX_ACCESS_TOKEN"), nil
    })
}
```

The provider runs before each request. Your application owns secure token storage, expiry checks, and refresh coordination; see [OAuth for REST APIs](/developer-tools/oauth#refresh-and-disconnect). The same token option is available for Sending and Management clients.
...
| --- | --- | --- | --- |
| Sending | `sendmux.ai/go/v3/sending` | `sending.New` | `smx_mbx_` or owner-approved `smx_agent_` with `email.send` |
| Mailbox | `sendmux.ai/go/v3/mailbox` | `mailbox.New` | `smx_mbx_` or scoped `smx_agent_` |
| Management | `sendmux.ai/go/v3/management` | `management.New` | `smx_root_` |

<Note>
  Durable `smx_agent_` tokens are mailbox-compatible and include `mailbox.read`
  and `email.receive`, not `email.send`. After owner acceptance and approval,
  exchange the durable token for a one-hour Sending-resource token before sending.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Management API" icon="chart-line" href="/api/introduction">
    Review the Management API contract used by `sendmux.ai/go/v3/management`.
  </Card>
