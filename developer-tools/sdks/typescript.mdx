...
Set `accessToken` to a bare token string or a synchronous or asynchronous provider. Do not also set `apiKey`.
...
The `@sendmux/ai-sdk` wrapper accepts the same `accessToken` option in `sendmux(...)`. To use all three tools, request `mailbox.read` and `email.send` and select one mailbox at consent; the tools do not supply a mailbox selector.
