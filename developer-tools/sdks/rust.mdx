...
Use `new_with_access_token` for a bare token or `new_with_token_provider` for an asynchronous provider.

```rust
use sendmux::mailbox::MailboxClient;

fn oauth_client() -> sendmux::Result<MailboxClient> {
    MailboxClient::new_with_token_provider(|| async {
        Ok(std::env::var("SENDMUX_ACCESS_TOKEN")
            .expect("SENDMUX_ACCESS_TOKEN is required"))
    })
}
```
