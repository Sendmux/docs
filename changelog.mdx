...
Sendmux-managed sending now pauses when complaints, repeated permanent bounces, or Amazon SES health findings require review. New managed sends are rejected while a team is held, queued messages remain paused, and sending resumes only after administrator review and a fresh healthy status check. Customer-managed sending accounts remain available.
