...
<Update label="7 July 2026" tags={["Improvements"]} rss={{ title: "Inbound attachments are easier for agents to read" }}>
  ## Mailbox API
...
MCP clients can now read text attachments directly through the mailbox attachment read tool, so agents no longer need a separate web fetch step for common files like Markdown recipes.
