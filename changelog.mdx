...
<Update label="24 August 2026" tags={["New features", "Improvements"]} rss={{ title: "New teams move to Free and Pro pricing immediately" }}>
  ## Free and Pro rollout for new teams

When you create a new team, it starts on Free and current per-event usage rates apply immediately. An owner can upgrade it to Pro. Team creation time, not account signup time, decides the initial cohort. Teams created before this rollout keep their existing pricing until 24 September 2026 at 02:00 UTC unless an owner upgrades them earlier.

Free includes two mailboxes and allows up to 50 provider-accepted recipients per UTC day across all sending routes. These limits are fixed, so if you need more capacity, delete an unused resource or upgrade to Pro.

</Update>

{/* Product source verified through Sendmux commit 33c2d4a. */}

<Update label="22 August 2026" tags={["New features", "Improvements"]} rss={{ title: "Free and Pro plans add fixed limits and per-event pricing" }}>
  ## Free and Pro plans

For legacy teams, Free includes two mailboxes and allows up to 50 provider-accepted recipients per UTC day across all sending routes from 24 September 2026 at 02:00 UTC. These limits are fixed, so if one is reached, the team owner must upgrade to Pro instead of requesting a higher allowance.

Pro costs \$7 per team each month, plus usage, with no plan-level resource count limits. Provider quotas, safety controls, and account health restrictions still apply. Outgoing mail through your own or connected provider costs \$0.000500 per provider-accepted recipient occurrence. Sendmux-managed Amazon SES sending costs \$0.000750 per accepted recipient. Inbound mail costs \$0.000500 per distinct mailbox delivery. Each event is billed individually, not in blocks of 1,000.

Existing Free teams stay on Free, but Free teams cannot purchase extra credit. When a Free balance is depleted, the owner must upgrade to Pro to continue sending. From the effective date, each account can own one active Free team; additional Free teams become read-only unless they move to Pro. Existing paid access, resources, and balances remain in place through the change.
...
Free accounts can create one team. Within that team, you can add up to one domain, one mailbox, one customer-managed sending account, one webhook, and two active credentials. Team access starts with the owner only. Existing resources remain in place; contact support or request an increase where the option is available if you need more capacity.
