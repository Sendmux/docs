...
Large connection tests continue in the background when they take longer than expected. Temporary interruptions while saving a completed result are retried, so closing the progress window does not leave an account pending.
