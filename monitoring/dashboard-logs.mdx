...
Applied delivery-group public IDs appear with the provider. The Management API exposes them as `delivery_group`, and CSV exports include a **Delivery Groups** column. The value is `null` in the API, or blank in the export, when no hint was applied, a shared-domain override ignored it, or the log predates this field. Later status updates preserve the applied IDs.
...
<Columns cols={2}>
  ...
  <Card title="Service status" icon="chart-line" href="https://status.sendmux.ai">
    Check current service availability and incident updates.
  </Card>
</Columns>
