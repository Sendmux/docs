...
description: "Configure Sendmux MCP in documented AI clients."
keywords:
  [
    "MCP clients",
    "sendmux-mcp",
...
<CardGroup cols={3}>
  <Card title="Hosted OAuth" icon="shield-check" href="#hosted-oauth-clients">
    Use this for Atlassian, Claude, Cursor, Codex, Gemini CLI, Qwen Code, Kiro, OpenCode, Zed, Visual Studio, Hoot, and ChatGPT.
  </Card>
  ...
  <Card title="Private HTTP" icon="network-wired" iconType="solid" href="#private-http-clients">
    Use this when your client needs a URL or runs outside your laptop.
  </Card>
</CardGroup>

<Accordion title="Compatibility matrix">
  These labels describe documented configurations. They do not claim automatic client certification unless a row says so.

  | Client | Recommended setup | Connection and validation |
  | --- | --- | --- |
  | Atlassian | Hosted OAuth | Custom external MCP server; documented, manual acceptance pending |
  | Claude and Claude Desktop | Hosted OAuth | Custom connector; documented |
  | Claude Code | Hosted OAuth | Remote HTTP, local stdio, private HTTP; documented |
  | Cursor | Hosted OAuth | Remote HTTP, local stdio, private HTTP; documented |
  | Codex | Hosted OAuth | Remote HTTP, local stdio, private HTTP; documented |
...
<Card title="Atlassian" icon="plug" href="/ai-integrations/mcp/clients/atlassian">
  Add Sendmux as a custom external MCP server in Atlassian administration.
</Card>
