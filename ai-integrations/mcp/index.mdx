...
import McpReleaseFacts from "/snippets/mcp-release-facts.mdx";
...
<CardGroup cols={2}>
  <Card title="Hosted OAuth" icon="shield-check" href="/ai-integrations/mcp/clients#hosted-oauth-clients">
    Use when your client can open a browser-based MCP OAuth flow.
  </Card>
  <Card title="Local stdio" icon="terminal" href="/ai-integrations/mcp/clients#local-stdio-clients">
    Use when your coding agent can launch a local command on your machine.
  </Card>
  <Card title="Private HTTP" icon="network-wired" iconType="solid" href="/ai-integrations/mcp/clients#private-http-clients">
...
<AccordionGroup>
  ...
  <Accordion title="Protected environment input">
    ```bash
    SENDMUX_MCP_SURFACES=mailbox,management,sending \
    SENDMUX_MAILBOX_API_KEY="$SENDMUX_MAILBOX_KEY" \
    SENDMUX_MANAGEMENT_API_KEY="$SENDMUX_ROOT_KEY" \
    SENDMUX_SENDING_API_KEY="$SENDMUX_SENDING_KEY" \
    sendmux-mcp
    ```
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="A2A" icon="network-wired" iconType="solid" href="/ai-integrations/a2a">
    Use Sendmux through an A2A 1.0 agent endpoint with hosted OAuth.
  </Card>
