...
API keys are scoped to a team and carry specific permissions. Create and manage keys under **Settings > API Keys** in the Sendmux app. Free includes **2 active credentials** across manual API keys and connected apps. Pro has no plan-level credential count limit. Manual keys include root keys, send-only keys, and mailbox-scoped keys. See [Team limits](/account/team-limits).
...
The shared Amazon SES account is protected. You can read, activate, or deactivate it. Pro teams can request a daily limit review. Free teams must upgrade for more capacity. You cannot edit its credentials, quotas, routing weight, sender defaults, or delete it through the API.
