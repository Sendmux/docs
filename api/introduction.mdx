...
Billing balance shows what you can spend now.
...
Authenticate using a root API key or a [REST OAuth access token](/developer-tools/oauth) with Management access. Pass it as a Bearer token in the `Authorization` header.
...
API keys are scoped to a team and carry permissions. Create and manage them under **Settings > API Keys** in the Sendmux app. New Free teams include **2 active credentials** across manual API keys and connected apps. Manual keys include root keys, send-only keys, and mailbox-scoped keys. Free limits are fixed; disconnect a credential or upgrade to Pro when you need more capacity. See [Team limits](/account/team-limits).

### Permissions

Resource endpoints require their listed permissions. `GET /me` validates a root API key or Management OAuth grant without an additional read permission; see [Test an integration connection](/developer-tools/integration-connections). Keys with wildcard permissions (e.g. `domain.*`) satisfy any permission in that namespace, and the built-in `root:full` role includes every wildcard.
...
The shared Amazon SES account is protected. You can read, activate, or deactivate it. Free limits are fixed. Eligible Pro teams can request a daily limit review at 80% usage. You cannot edit its credentials, quotas, routing weight, sender defaults, or delete it through the API.
...
You can also import the [Management API Postman collection](/developer-tools/postman#choose-a-collection) by URL.
