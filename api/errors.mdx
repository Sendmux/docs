...
<Card title="Service status" icon="chart-line" href="https://status.sendmux.ai">
  Check current service availability before retrying persistent `503` responses.
</Card>
...
**HTTP 403**: The API key is valid but lacks the required permission for this endpoint. Check the [permission table](/api/introduction#permissions) to see which permission each endpoint requires.
...
**HTTP 409**: Your team has hit the create limit for this resource. The limit can apply to domains, mailboxes, sending accounts, webhooks, or active API keys. A Free team must delete an existing item or upgrade to Pro before retrying. See [Team limits](/account/team-limits).

### idempotency_conflict

**HTTP 409**: Returned when an `Idempotency-Key` request header is reused while the original request is still running, or with a request body whose fingerprint differs from the original. Wait and retry for an in-flight request. Use a fresh key for a different request. See the [Idempotency](/sending/idempotency) guide.
