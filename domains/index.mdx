...
<Note>
  If you're using a Free team, you can have **1 domain**. Check your current
  limit on the **Domains** page. Free limits are fixed. If you need more
  capacity, delete an unused domain or upgrade to Pro. See [Team
...
<AccordionGroup>
  ...
  <Accordion title="Bounce handling is failing">
    Check the MX and TXT records on the `bounce.{your-domain}` subdomain. The
    MX record must use the target Sendmux shows, and the TXT record must match
    the generated SPF value.
  </Accordion>
...
<Columns cols={2}>
  ...
  <Card title="Domain settings" icon="settings" href="/domains/settings">
