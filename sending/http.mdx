...
On Free, accepted and in-flight recipients share a limit of 10 per UTC day. When a request would cross it, Sendmux returns `429 daily_limit_exceeded` before queueing that message. Wait for the `Retry-After` period or upgrade to Pro.
