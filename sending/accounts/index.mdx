...
| Setting               | Free                   | Pro                       |
| --------------------- | ---------------------- | ------------------------- |
| Starting hourly limit | 2 accepted recipients  | 100 accepted recipients   |
| Starting daily limit  | 50 accepted recipients | 1,000 accepted recipients |
| Daily reset           | 00:00 UTC              | 00:00 UTC                 |
| Setup                 | None                   | None                      |
| Editable credentials  | No                     | No                        |
| Delivery groups       | Not supported          | Not supported             |

<SharedSesPolicy />

Free managed-sending limits are fixed. Upgrade to Pro for more capacity. On Pro, **Request increase** appears on **Accounts** at 80% of the managed Amazon SES daily limit. You can also call `POST /providers/shared-amazon-ses-limit-request` from the Management API. Submit the request once; Sendmux reviews it and emails the outcome.
...
<AccordionGroup>
  <Accordion title="CSV column reference">
    ...
    | `from_email`, `from_name` | No | Sender address applied to every message sent through this account, and the display name used when a message has none. `from_email` applies only to SMTP accounts; connected accounts use `oauth_user_email`. |
    | `reply_to_email`, `reply_to_name` | No | Reply-To address applied to every message sent through this account, and the display name used when a message's Reply-To has none. |
...
- Search or filter by status, group, or type. Select the visible page, every
  matching account, or all accounts, then activate, deactivate, update quotas,
  update tracking, change groups, update variables, or delete the selection.
  Use the **Accounts** menu to export all custom accounts. Large changes
  continue in the background and show completed and failed account counts.
...
<Warning>
  The shared Amazon SES account is managed by the platform. API clients can
  read it, activate or deactivate it, and request a daily limit review for an
  eligible Pro team. They cannot edit its credentials, quotas,
  routing weight, or sender defaults, and cannot delete it. You can edit its
  template variables. Free limits are fixed.
</Warning>
...
| Setting                     | Use it for                                                                     |
| --------------------------- | ------------------------------------------------------------------------------ |
| **From email**              | Replace the sender address on every message sent through this SMTP provider.  |
| **From name**               | Set the sender display name used when a message has none.                     |
...
Edit an account's **Variables** to store values such as `signature_url`. Insert `[DP_VAR_SIGNATURE_URL]` into a subject or body to use the value from the provider selected for that message.

To edit several accounts, select them and open **Update Variables**. Choose
**Add or update** to add supplied keys and overwrite matching keys, **Replace
all** to replace each map, or **Remove** to delete named keys. Select **Apply
changes** to start the update. Each resulting map must satisfy the limits.
Review any reported failures after applying the edit.

See [Template variables](/sending/template-variables) for the key rule, value limits, built-in tags, raw insertion behaviour, and Management API examples.

## Account limits

| Plan | Your own sending accounts | More capacity |
| ---- | ------------------------- | ------------- |
| Free | 1; the managed account does not count | Delete an account or upgrade to Pro |
| Pro  | No plan-level count limit | Provider quotas and safety controls still apply |
