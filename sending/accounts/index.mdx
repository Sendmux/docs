...
| Starting daily limit  | 10 accepted recipients | 1,000 accepted recipients |
...
Free limits are fixed. When a Free team reaches a managed sending limit, upgrade to Pro. On Pro, **Request increase** appears on **Accounts** at 80% of the daily limit. You can also call `POST /providers/shared-amazon-ses-limit-request` from the Management API.
...
<Note>
  Free teams can add one account of their own. Pro has no plan-level sending
  account count limit. Updating an existing account does not add another
  account. Only one import can run for a team at a time.
</Note>
...
| Request more custom-account capacity on an eligible paid team | `POST /providers/limits/sending-accounts/request-increase`                         |
...
<Warning>
  ...
  activate or deactivate it. Pro teams can request a daily limit review. Free
  teams must upgrade for more capacity. API clients cannot edit its credentials,
  quotas, routing weight, sender defaults, or delete it.
</Warning>
...
Free teams can add **one** account of their own. Pro has no plan-level sending account count limit. The managed account does not count towards this limit.
