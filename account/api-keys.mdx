...
| **Agent access token** | An agent registers through agent access.         | Durable for reads; one hour for delegated sending.    | Agent access flow, then the **Agent access** card on the team page after owner acceptance. |
...
The durable agent token includes `mailbox.read` and `email.receive`, not `email.send`. After the owner joins and approves sending, the agent can exchange it for a one-hour Sending-resource `smx_agent_` token through [agent access](/ai-integrations/agent-access).

Owners approve or revoke agent sending from the **Agent access** card on the
team page after accepting the agent invite.

<Note>
  New Free teams include **2 active credentials** across manual API keys and
  connected apps. Free limits are fixed. Disconnect a credential or upgrade to
  Pro when you need more capacity. See [Team limits](/account/team-limits).
</Note>
...
<Steps>
  ...
  <Step title="Disconnect an app">
    Select the disconnect action beside the app. This revokes its refresh token
    and grant immediately.
  </Step>
</Steps>
...
<Steps>
  ...
  <Step title="Store the secret">
    Copy the generated `smx_root_` or `smx_mbx_` value. Sendmux will not show it
    again.
  </Step>
</Steps>
